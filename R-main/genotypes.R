...
# Filter the vcf file
system('/usr/local/bin/vcflib/vcffilter -f "TYPE = snp & DP > 300 & QUAL > 20" inst/extdata/satro48_p1_noMNP_noComplex_noPriors.vcf > intermediates/sebastes48_p1_snps_filtered.vcf')
...
system('/usr/local/bin/vcftools --vcf intermediates/sebastes48_allSNPs_filtered.vcf --out intermediates/sebastes --012')
...
cat("Title line:\"sebastes48_allSNPs.genepop\"", "\n", sep = "", file = "output/sebastes48_allSNPs.genepop")
cat(gsMarkers, sep = "\n", file = "output/sebastes48_allSNPs.genepop", append = TRUE)
write.table(genos, col.names = FALSE, quote = FALSE, sep = " ", file = "output/sebastes48_allSNPs.genepop", append = TRUE)
...
cat("Inds", twocolMarkers, "\n", sep = "\t", file = "output/sebastes48_p1_filtered_noAB_TK.txt")
write.table(genos, col.names = FALSE, quote = FALSE, sep = "\t", file = "output/sebastes48_p1_filtered_noAB_TK.txt", append = TRUE)
